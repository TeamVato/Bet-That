...
@pytest.mark.skip(
    reason="Schedule fallback functionality removed from current edge engine implementation"
)
def test_edge_engine_uses_schedule_fallback_for_missing_week(tmp_path):
    ...
    assert len(edges_df) == 2  # Now generates both over and under edges

    # Verify week was populated from schedule lookup for both edges
    assert all(edges_df["week"] == 19)
    assert all(edges_df["season"] == 2025)


@pytest.mark.skip(
    reason="Schedule fallback and opponent_def_code functionality removed from current edge engine implementation"
)
def test_edge_engine_uses_schedule_fallback_for_missing_opponent_def_code(tmp_path):
    ...
    # Verify opponent_def_code was inferred from event_id parsing for both edges
    # BAL-PIT event, BAL is offense, so PIT should be defense
    assert all(edges_df["opponent_def_code"] == "PIT")
...
def test_edge_engine_schedule_fallback_with_alternative_key_format(tmp_path):
    ...
    # Both should be populated from schedule for all edges
    assert all(edges_df["week"] == 19)
    assert all(edges_df["opponent_def_code"] == "PIT")
...
def test_edge_engine_preserves_existing_values_over_schedule_fallback(tmp_path):
    ...
    # Should preserve existing values, not use schedule fallback for all edges
    assert all(edges_df["week"] == 19)  # Not 18 from schedule
    assert all(edges_df["opponent_def_code"] == "PIT")
...
def test_edge_engine_handles_missing_schedule_lookup_gracefully(tmp_path):
    ...
    # Values should remain missing without schedule lookup for all edges
    assert all(pd.isna(edges_df["week"]))
