...
def test_apply_season_filter_without_season_column_returns_original():
    ...
    filtered = apply_season_filter(df, [2025])
    assert filtered.equals(df)

    filtered_no_selection = apply_season_filter(df, [])
