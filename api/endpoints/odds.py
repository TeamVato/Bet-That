...
from ..schemas import OddsResponse, OddsBestLinesResponse
...
async def get_best_odds(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch odds data: {str(e)}"
        )
...
async def get_event_odds(
    ...
    """
    try:
        odds_lines = db.query(CurrentBestLine).filter(
            CurrentBestLine.event_id == event_id
        ).all()
