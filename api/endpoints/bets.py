...
async def create_user_bet(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create bet: {str(e)}"
        )
...
async def get_user_bets(
    ...
    try:
        ...
        # Query user bets
        bets_query = db.query(UserBet).filter(
            UserBet.external_user_id == user["external_id"]
        ...
        bet_responses = []
        for bet in bets:
            bet_responses.append(UserBetResponse(
                id=bet.id,
                game_id=bet.game_id,
                market=bet.market,
