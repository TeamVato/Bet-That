...
class EdgeEngine:
    ...
    def _prepare_dataframe(
        ...
        print(
            f"DEBUG: _prepare_dataframe - props: {len(props_df)}, projections: {len(projections_df)}"
        )
        ...
        if "market" in merged.columns:
            ...
            if supported_count == 0:
                ...
                print(
                    f"DEBUG: Filtered to {len(merged)}/{before_filter} rows with supported markets"
                )
    ...
    def compute_edges(self, props_df: pd.DataFrame, projections_df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(
            f"DEBUG: Edge computation input - props: {len(props_df)} rows, projections: {len(projections_df)} rows"
        )
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(over_odds) or pd.isna(under_odds) or pd.isna(line):
                if processed_count <= 5:  # Log first few failures
                    print(
                        f"DEBUG: Row {idx} skipped - missing data: over_odds={over_odds}, under_odds={under_odds}, line={line}"
                    )
                continue
            ...
            try:
                ...
                kelly_over = min(
                    self.config.kelly_cap, odds_math.kelly_fraction(p_over, over_odds_int)
                )
                kelly_under = min(
                    self.config.kelly_cap, odds_math.kelly_fraction(p_under, under_odds_int)
                )
        ...
        print(
            f"DEBUG: Processed {processed_count} rows, {error_count} errors, generated {len(rows)} edge records"
        )
    ...
    def export(self, edges_df: pd.DataFrame) -> None:
        ...
        try:
            export_path = (
                self.config.export_dir / f"edges_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )
