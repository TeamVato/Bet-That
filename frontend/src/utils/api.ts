...
export const api = {
  ...
  createBet: (payload: any) => {
    if (BETA_VIEW_ONLY) {
      throw new ApiError('Bet creation disabled in beta view-only mode.', 403, { reason: 'view_only' })
    }
    return request('/me/bets', { method:'POST', body: JSON.stringify(payload) }, true)
  },
