...
export const BetSlip: React.FC<BetSlipProps> = ({
  ...
  const handlePlaceBet = async () => {
    if (viewOnly) {
      setError(
        "Bet placement is disabled while we are in beta view-only mode.",
      );
      return;
    }
    ...
    try {
      const selection = edge.recommendation.includes("UNDER")
        ? "UNDER"
        : "OVER";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        ...
        {viewOnly && (
          <div
            className="mb-4 rounded border-l-4 border-yellow-500 bg-yellow-50 p-3 text-sm text-yellow-800"
            role="note"
          >
            {safetyNotice}
          </div>
