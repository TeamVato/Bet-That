...
const statusBadge: Record<EdgeStatus, string> = {
  ...
  monitor:
    "bg-amber-400/20 text-amber-600 dark:bg-amber-400/20 dark:text-amber-200",
  "no-action":
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Tracking totals movement across top U.S. books.
          </p>
          {lastFetchedLabel && (
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-medium text-gray-900 dark:text-gray-100">
                Double-Edge Opportunities
              </h3>
              <span
                className="text-xs text-gray-500 dark:text-gray-400"
                title="Edges flag 2+ point gaps between books. That is usually when arbitrage windows open."
              >
            ...
            {edges.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-300 bg-white/40 p-6 text-sm leading-relaxed text-gray-600 dark:border-slate-700 dark:bg-slate-900/40 dark:text-gray-300">
                No edges right now — totally normal. Markets usually pop closer
                to kickoff. Keep tracking movement or set alerts.
              </div>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                {edges.map((edge) => (
                  <EdgeCard key={edge.gameId} edge={edge} />
                ))}
              </div>
            )}
          </section>
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {games.map((game) => (
                <article
                  key={game.id}
                  className="card p-4 bg-white/80 dark:bg-slate-900/70"
                >
                  <header className="flex items-start justify-between gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusBadge[game.status]}`}
                    >
                  ...
                  <dl className="mt-3 space-y-2 text-xs text-gray-600 dark:text-gray-300">
                    <div
                      ...
                      title="Total gap between books"
                    >
                      <dt className="uppercase tracking-wide text-gray-500 dark:text-gray-400">
                        Movement
                      </dt>
                      <dd className="font-semibold text-gray-900 dark:text-gray-100">
                        {game.movement.toFixed(1)} pts
                      </dd>
                    </div>
                    {game.lowestTotalPoint !== undefined &&
                      game.highestTotalPoint !== undefined && (
                        <div className="flex flex-wrap gap-x-4 gap-y-1">
                          <div>
                            ...
                            <dd>
                              {game.lowestTotalPoint.toFixed(1)} (
                              {game.lowestBook})
                            </dd>
                          </div>
                          <div>
                            ...
                            <dd>
                              {game.highestTotalPoint.toFixed(1)} (
                              {game.highestBook})
                            </dd>
                          </div>
                        </div>
                    ...
                    {game.keyNumbers.length > 0 && (
                      <div
                        className="flex items-center gap-1 text-amber-600 dark:text-amber-300"
                        title="Totals close to key football numbers demand extra caution."
                      >
                  ...
                  >
                    {game.recommendation ??
                      "Keep tracking for double-edge swing."}
                  </p>
                </article>
              ))}
            </div>
