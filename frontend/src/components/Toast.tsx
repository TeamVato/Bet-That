...
export default function Toast({ message, type='info', onClose }:{ message:string, type?:'info'|'error'|'success', onClose:()=>void }){
  ...
  useEffect(()=>{ const t=setTimeout(()=>{setOpen(false); onClose()}, 3000); return ()=>clearTimeout(t)},[])
  if(!open) return null
  ...
  return <div className={`fixed right-4 bottom-4 border rounded-lg px-3 py-2 text-sm shadow ${color}`}>{message}</div>
}
